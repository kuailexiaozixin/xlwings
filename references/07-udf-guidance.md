...
> 本文件是 SKILL.md 6.5.7（UDF 开发与导入）的扩展资源。权威语义与全部装饰器参数见 `xlwings-0.37.0/docs/udfs.md`。UDF 侧开发以本文档为准。
...
- **`xlwings-0.37.0/examples/udf/udf.py`**（最小 UDF 集）：`@xw.sub` 的 `get_workbook_name`（宏入口）+ `@xw.func` 函数（含 numpy/pandas 转换）——UDF 编写的最小对照；`udf.xlsm` 为已装配工作簿；
- **`xlwings-0.37.0/examples/fibonacci/`**（UDF + 便携分发）：`fibonacci.py`（`@xw.func` 斐波那契）+ `build_standalone.py`（PyInstaller 冻结独立可执行 → zip 分发）+ `setup_fibonacci.py`（冻结配置）——场景 C 6.5.10 便携运行时分发的官方样例；
...
- **`xlwings-0.37.0/docs/udfs.md`（权威，全部装饰器参数与语义）**：读 UDF 前必读
- `xlwings-0.37.0/docs/converters.md`：类型转换体系（UDF 入参/返回转换的底层）
- `examples/Excel_udf_itus-main/`：UDF 教程（类型提示/二维数组/DataFrame 实战）
- `xlwings-0.37.0/docs/threading_and_multiprocessing.md`：异步 UDF 的线程语义
