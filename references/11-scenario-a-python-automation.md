...
**本质**：`chunksize` 只控制内部 COM 调用的批次大小，不改变返回类型。源码见 `xlwings-0.37.0/xlwings/conversion.py` 中 `Range.value` 的 getter 逻辑——分批调用 `GetValue` 后 `extend` 到同一个 list。
...
完整的 MSO_SHAPE_TYPE 枚举有 100+ 种，常用的：矩形(1)、圆角矩形(5)、椭圆(9)、三角形(7)、菱形(4)、箭头(33/34)、标注(51+)。需要时查 `xlwings-0.37.0/xlwings/constants.py` 或微软文档。
...
详细说明见 `xlwings-0.37.0/docs/threading_and_multiprocessing.md`。
...
### 7.1 官方 quickstart 迷你示例（xlwings-0.37.0/examples/，BSD）
